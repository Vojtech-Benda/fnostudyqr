...
int main(int argc, char* argv[]) {
    ...
    const char *APP_RELEASE_DATE{ "5.1.2025" };
    ...
    const char *        opt_peerIP{ nullptr }; // hostname/ip address of PACS (DICOM peer)
    ...
    cmd.addOption("--ae-caller", "-aec", 1,
        "[a]etitle: string",
        fmt::format("set my calling AE title (default: {}", APPLICATION_TITLE).c_str());
    ...
    cmd.addOption("--overwrite-modality-all", "-oma",
        "overwrite modalities of all read patient records");
    ...
    if (app.parseCommandLine(cmd, argc, argv))
    {
        ...
        cmd.getParam(1, opt_peerIP); // ip address of PACS
        queryRetriever.m_calledIP = opt_peerIP;

        app.checkParam(cmd.getParamAndCheckMinMax(2, opt_peerPort, 1, 65535));
        queryRetriever.m_port = OFstatic_cast(unsigned short, opt_peerPort);
        ...
        {
            //app.checkDependence("--output-directory", "--port", opt_recievePort > 0);
        ...
        {
            OFLOG_ERROR(mainLogger, fmt::format("Missing parameter --receiver-port (-port) number"));
		...
		{
			OFLOG_ERROR(mainLogger, "No text file specified, exiting program");
			return EXITCODE_COMMANDLINE_SYNTAX_ERROR;
		}
    }
    ...
    {
        OFLOG_ERROR(mainLogger, fmt::format("Text file {} not found, exiting program", filepath.string()));
        return EXITCODE_TEXT_FILE_ERROR;
    }

    fmt::print("READING TEXT FILE ------------------------- \n");
    //PatientRecordMap recordMap = readPatientRecords(filepath);
    ...
    {
        OFLOG_FATAL(mainLogger, "Record map is empty, exiting program");
    ...
    {
        OFLOG_ERROR(mainLogger, "No modalities to query for, exiting program");
    ...
    std::ofstream missingStudiesFile{ missingStudiesFilename, std::ios::out | std::ios::trunc };
    if (missingStudiesFile.is_open())
    {
        missingStudiesFile << fmt::format("{:%Y-%m-%d %H:%M:%S}\n", tm);
		missingStudiesFile << "PatientID, StudyDate\n";
    }
    ...
    for (auto& record : recordList)
    {
        ...
        if (record.m_uid_list.empty())
        {
            ...
            if (missingStudiesFile.is_open())
                missingStudiesFile << fmt::format("{}, {} - NOT FOUND\n", record.m_id, record.m_study_date);
        }
        else
        {
            fmt::print("{} - {}\n", msg, fmt::format(fg(fmt::color::green), "SUCCESS, {} study/ies", record.m_uid_list.size()));
