...
{
	T_ASC_NetworkRole role = (this->m_retrievePort > 0) ? NET_ACCEPTORREQUESTOR : NET_REQUESTOR;
...
OFCondition QueryRetriever::setupAssociation()
{
	...
	if (cond.bad())
	{
		OFLOG_FATAL(qrLogger, "Adding C-FIND presentation contexts failed: " << DimseCondition::dump(temp_string, cond));
		(void)ASC_destroyAssociationParameters(&this->m_params);
		return cond;
	}
	...
	if (cond.bad())
	{
		OFLOG_FATAL(qrLogger, "Adding C-MOVE presentation contexts failed: " << DimseCondition::dump(temp_string, cond));
		(void)ASC_destroyAssociationParameters(&this->m_params);
		return cond;
	}

	OFLOG_DEBUG(qrLogger, "Request parameters: " << OFendl << ASC_dumpParameters(temp_string, this->m_params, ASC_ASSOC_RQ));
	...
	OFLOG_DEBUG(qrLogger, "Association parameters negotiated: " << OFendl << ASC_dumpParameters(temp_string, this->m_params, ASC_ASSOC_AC));
...
OFCondition QueryRetriever::performFindRequest(PatientRecord& patient_record, const std::string& modalities, QueryCallback* callback) const {
	...
	while (cond.good() && repeatCount--)
	{
		...
		cond = DIMSE_queryUser(this->m_assoc, presID,
							   &request, requestedDataset,
							   responseCount,
...
OFCondition QueryRetriever::performMoveRequest(const PatientRecord& patient_record)
{
	...
	std::strncpy(request.AffectedSOPClassUID, this->m_abstractSyntax.moveSyntax,
	...
	for (const auto& uid : patient_record.m_uid_list)
	{
		...
		std::string studyDirectory = fmt::format("{}/{}", this->m_outputDirectory, uid);

		if (std::filesystem::exists(studyDirectory))
			fmt::print("Study directory {} exits, overwriting\n", studyDirectory);

		(void)std::filesystem::create_directories(studyDirectory);

		cond = DIMSE_moveUser_(this->m_assoc, presID,
							  &request, requestedDataset,
							  moveCallback, &moveCallbackInfo,
							  this->m_blockMode, this->m_dimseTimeout,
							  this->m_net,
							  subOpCallback, nullptr,
							  &response,
		...
		if (cond == EC_Normal)
		{
				...
				(response.DimseStatus == STATUS_MOVE_Cancel_SubOperationsTerminatedDueToCancelIndication))
			{
				...
				const std::string msg = fmt::format("PatientID: {}, StudyDate: {}, StudyUID: {}", patient_record.m_id, patient_record.m_study_date, uid);
			...
			else if (response.DimseStatus == STATUS_MOVE_Warning_SubOperationsCompleteOneOrMoreFailures)
			{
				...
				OFLOG_WARN(qrLogger, "Move response with warning status (" << DU_cmoveStatusString(response.DimseStatus) << ")");
			}
			else
			{
				cmove_status_code = EXITCODE_CMOVE_ERROR;
				OFLOG_WARN(qrLogger, "Move response with error status (" << DU_cmoveStatusString(response.DimseStatus) << ")");
			}
			...
			if (qrLogger.isEnabledFor(OFLogger::DEBUG_LOG_LEVEL))
			{
				...
				if (responseIDs != nullptr)
					...
					OFLOG_INFO(qrLogger, "Received Final Move Response (" << DU_cmoveStatusString(response.DimseStatus) << ")");
			}
		}
		else
		{
...
void QueryCallback::setPresentationContextID(T_ASC_PresentationContextID presID)
{
	this->m_presID = presID;
}
									...
									std::vector<std::string>& uid_list)
{
	...
	{
		OFLOG_INFO(qrLogger, "---------------------------");
		OFLOG_INFO(qrLogger, "Find Response: " << responseCount << " (" << DU_cfindStatusString(response->DimseStatus) << ")");
		OFLOG_INFO(qrLogger, DcmObject::PrintHelper(*responseIdentifiers));
	}
	else
	{
		DCMNET_INFO("Received Find Response " << responseCount << " (" << DU_cfindStatusString(response->DimseStatus) << ")");
	}

	OFString studyuid;
	if (responseIdentifiers->findAndGetOFString(DCM_StudyInstanceUID, studyuid).good())
	{
		if (!studyuid.empty())
			uid_list.emplace_back(studyuid.c_str());
	}

	if (this->m_cancelAfterNResponses == responseCount)
	{
		DCMNET_INFO("Sending Cancel Request (MsgID " << request->MessageID << ", PresID " << OFstatic_cast(unsigned int, this->m_presID) << ")");
...
static void progressCallback(void* callbackData,
							 ...
							 std::vector<std::string>& uid_list)
{
	QueryCallback* callback = OFreinterpret_cast(QueryCallback*, callbackData);
	if (callback)
		callback->callback(request, responseCount, response, responseIdentifiers, uid_list);
}
...
OFCondition DIMSE_queryUser(T_ASC_Association* assoc,
							...
							int responseCount,
							DIMSE_QueryUserCallback callback, void* callbackData,
							...
							std::vector<std::string>& uid_list)
{
	...
	if (requestIdentifiers == nullptr) return DIMSE_NULLKEY;
	...
	while (cond == EC_Normal && DICOM_PENDING_STATUS(status))
	{
		...
		{
			std::string buf{ fmt::format("DIMSE: Unexpected Response Command Field: {:#04x}",
		...
		*response = rsp.msg.CFindRSP;
		...
		{
			std::string buf{ fmt::format("DIMSE: Unexpected Response MsgID: {} (expected: {})",
		...
		case STATUS_FIND_Pending_WarningUnsupportedOptionalKeys:
			...
			if (callback)
				callback(callbackData, request, responseCount, response, rspIDs, uid_list);
		...
		default:
			if (response->DataSetType != DIMSE_DATASET_NULL)
			{
				DCMNET_WARN(DIMSE_warn_str(assoc) << "findUser: Status " << DU_cfindStatusString(status) << ", but DataSetType != nullptr");
				DCMNET_WARN(DIMSE_warn_str(assoc) << "Assuming no response identifiers are present");
			}
			break;
		}// switch end
...
OFCondition DIMSE_moveUser_(T_ASC_Association* assoc,
						   ...
						   int dimse_timeout,
						   T_ASC_Network* net,
						   DIMSE_SubOpProviderCallback_ subOpCallback,
						   ...
						   std::string& output_directory)
{
	...
	while (cond == EC_Normal && status == STATUS_MOVE_Pending_SubOperationsAreContinuing)
	{
		...
		case 0:
			...
			{
				DCMNET_DEBUG(fmt::format("Timeout of {} seconds elapsed while waiting for C-MOVE Responses", dimse_timeout));
		...
		case 2:
			...
			if (subOpCallback)
			{
				subOpCallback(subOpCallbackData, net, &subAssoc, output_directory, block_mode, dimse_timeout);
			}
			firstLoop = OFFalse;
			continue; // continue with main loop
		}
		...
		{
			std::string buf{ fmt::format("DIMSE: Unexpected Response Command Field: {:#04x}", static_cast<unsigned>(rsp.CommandField)) };
			fmt::print("{}", buf);
			return makeDcmnetCondition(DIMSEC_UNEXPECTEDRESPONSE, OF_error, buf.c_str());
		}
		...
		*response = rsp.msg.CMoveRSP;
		...
		if (response->MessageIDBeingRespondedTo != msgID)
		{
			std::string buf{ fmt::format("DIMSE: Unexpected Response MsgID : {} (expected {})", response->MessageIDBeingRespondedTo, msgID) };
			fmt::print("{}", buf);
			return makeDcmnetCondition(DIMSEC_UNEXPECTEDRESPONSE, OF_error, buf.c_str());
		}
		status = response->DimseStatus;
			...
			if (response->DataSetType != DIMSE_DATASET_NULL)
			{
				...
				if (!ignore_pending_datasets)
				{
					...
					*/
					...
					cond = DIMSE_receiveDataSetInMemory(assoc, block_mode, dimse_timeout, &pres_id, &tempset, nullptr, nullptr);
					delete tempset;
		...
		default:
			if (response->DataSetType != DIMSE_DATASET_NULL)
			{
				cond = DIMSE_receiveDataSetInMemory(assoc, block_mode, dimse_timeout, &pres_id, response_ids, nullptr, nullptr);
				if (cond != EC_Normal)
					return cond;
			}
			break;
