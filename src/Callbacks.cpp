				  ...
				  T_DIMSE_C_MoveRSP *response)
{
	...
	{
		DCMNET_INFO("Sending cancel request (MsgID " << request->MessageID << ", PresID " << OFstatic_cast(unsigned int, moveCallbackInfo->presID) << ")");
							...
							int dimse_timeout)
{
	...
	if (cond == EC_Normal)
	{
		...
		case DIMSE_C_STORE_RQ:
			cond = storeSCP(*sub_assoc, &message, presID,
							output_directory,
	...
	if (progress->state == DIMSE_StoreEnd)
	{
		*status_detail = nullptr;
		if ((in_dataset != nullptr) && (*in_dataset != nullptr))
		{
			...
			E_TransferSyntax xfer = (*in_dataset)->getOriginalXfer();
			//if (xfer == EXS_Unknown)
					 ...
					 int dimse_timeout)
{
	...
	char filename[2048];
	OFStandard::snprintf(filename, sizeof(filename), "%s.%s",
